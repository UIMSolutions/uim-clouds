module uim.google.distributedcloud.helpers;
...
string buildResourcePath(string projectId, string location, string resource) {
  if (location.length > 0) {
    return format("projects/%s/locations/%s/%s", projectId, location, resource);
  }
  return format("projects/%s/%s", projectId, resource);
}
